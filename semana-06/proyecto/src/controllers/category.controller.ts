...
import { createCategorySchema, updateCategorySchema, idParamSchema } from '../schemas/category.schema';
...
export async function update(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    const category = await service.update(paramsResult.data.id, result.data);
    res.json({ data: category });
  } catch (err) {
    next(err);
  }
